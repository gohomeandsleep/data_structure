...
void Chain<T>::Add(const T& element) //add a new node after first
{
	...
	while (p && element.exp < p->data.exp) {
		q = p;
		p = p->link;
	}
...
public:
	...
	void display();
	template <typename U>
	friend ostream& operator<<(ostream& os, Polynomial<U>& poly);
	/*
...
Polynomial<T> Polynomial<T>::operator+(const Polynomial<T>& b) const {
	...
	while (ai.NotNull() && bi.NotNull()) {
		Term<T> termA = *ai, termB = *bi;

		if (termA.exp > termB.exp) {
			c.add(termA.coef, termA.exp);
			ai++;
		}
		else if (termA.exp < termB.exp) {
			c.add(termB.coef, termB.exp);
			bi++;
		}
		else {
			T newCoef = termA.coef + termB.coef;
			if (newCoef != 0) {
				c.add(newCoef, termA.exp);
			}
			ai++;
			bi++;
		}
...
int main(void) {
	...
	cout << "명령 선택: a: Add_a, b: Add_b, p: a + b, d: DisplayAll, q: exit : ";
	cin >> select;
	while (select != 'q')
	{
		switch (select)
		...
		cout << "Select command: a: Add_a, b: Add_b, p: Plus, d: DisplayAll, q: exit : ";
		cin >> select;
	}
	return 0;
}